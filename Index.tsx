...
const monthKey = (monthName: string) => monthName.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
...
type MonthRow = { month: string; revenue: number; meetings: number; clients: number; lost: number; ticket: number };
type RevenueBase = {
  monthSheet: string;
  totalRevenue: number;
...
const DEFAULT_STATE: AppState = {
  ...
  months: [
    { month: "Janeiro", revenue: 0, meetings: 0, clients: 0, lost: 0, ticket: 0 },
    { month: "Fevereiro", revenue: 0, meetings: 0, clients: 0, lost: 0, ticket: 0 },
    { month: "Março", revenue: 0, meetings: 0, clients: 0, lost: 0, ticket: 0 },
    { month: "Abril", revenue: 0, meetings: 0, clients: 0, lost: 0, ticket: 0 },
    { month: "Maio", revenue: 0, meetings: 0, clients: 0, lost: 0, ticket: 0 },
    { month: "Junho", revenue: 0, meetings: 0, clients: 0, lost: 0, ticket: 0 },
    { month: "Julho", revenue: 0, meetings: 0, clients: 0, lost: 0, ticket: 0 },
    { month: "Agosto", revenue: 0, meetings: 0, clients: 0, lost: 0, ticket: 0 },
    { month: "Setembro", revenue: 0, meetings: 0, clients: 0, lost: 0, ticket: 0 },
    { month: "Outubro", revenue: 0, meetings: 0, clients: 0, lost: 0, ticket: 0 },
    { month: "Novembro", revenue: 0, meetings: 0, clients: 0, lost: 0, ticket: 0 },
    { month: "Dezembro", revenue: 0, meetings: 0, clients: 0, lost: 0, ticket: 0 },
  ],
};
...
const rowsToRevenueBase = (rows: unknown[][], monthSheet: string): RevenueBase => {
  ...
  rows.forEach((row) => {
    ...
    const contract = row[2];

    if (!fee || fee <= 0) return;

    totalRevenue += fee;

    if (name && isRecurringContract(contract)) {
...
function NumberField({ value, onChange, prefix }: { value: number; onChange: (value: number) => void; prefix?: string }) {
  return (
...
function MoneyField({ value, onChange, hidden }: { value: number; onChange: (value: number) => void; hidden?: boolean }) {
...
function StatCard({ label, value, helper, hidden }: { label: string; value: string; helper?: string; hidden?: boolean }) {
  return (
...
function FunnelCard({ label, title, value, helper, hidden }: { label: string; title: string; value: string; helper?: string; hidden?: boolean }) {
  return (
...
export default function Index() {
  ...
  const setBR = (key: keyof Market, value: number) => setState((current) => ({ ...current, br: { ...current.br, [key]: value } }));
  const setUS = (key: keyof Market, value: number) => setState((current) => ({ ...current, us: { ...current.us, [key]: value } }));
  ...
  if (authLoading) {
    return (
      <main className="login-screen">
        <div className="login-card"><span className="brand-dot">Speedy Media OS</span><h1>Carregando dashboard...</h1></div>
      </main>
    );
  }
      ...
      <div className="page-wrap">
        <header className="hero">
          ...
          <div className="hero-actions">
            <label className="month-select"><span>Mês</span><select value={state.selectedMonth} onChange={(event) => set("selectedMonth", event.target.value)}>{MONTH_OPTIONS.map((month) => <option key={month} value={month}>{month}</option>)}</select></label>
            <button className="ghost-button compact" onClick={() => setShowValues(!showValues)} type="button">{showValues ? <EyeOff size={14} /> : <Eye size={14} />} {showValues ? "Ocultar" : "Mostrar"}</button>
            <button className="primary-button compact" onClick={() => window.print()} type="button"><FileDown size={14} /> PDF</button>
            <button className="ghost-button compact" onClick={() => void syncGoogleSheets()} type="button"><RefreshCw size={14} className={syncing ? "spin" : ""} /> Atualizar</button>
            <button className="ghost-button compact" onClick={() => navigator.clipboard.writeText("Resumo Speedy: " + BRL(state.revenueNow) + " de " + BRL(state.revenueGoal))} type="button"><Copy size={14} /> Resumo</button>
        ...
        <section className="revenue-panel">
          ...
          <div className="revenue-progress"><ProgressBar value={revenuePct} /><span>{hidden ? "***" : revenuePct + "% da meta"}</span></div>
        ...
        <section className="focus-panel">
          ...
          <div className="focus-number"><span>Faltam reuniões</span><strong>{hidden ? "***" : meetingGap}</strong><small>{hidden ? "***" : meetingsNow + " feitas de " + state.meetingsGoal + " planejadas"}</small><ProgressBar value={pct(meetingsNow, state.meetingsGoal)} /></div>
        </section>
        ...
        <section className="grid-two">
          <article className="panel">
            ...
            <div className="task-list">
              {state.tasks.map((task) => (
                <button key={task.id} className={task.done ? "task done" : "task"} type="button" onClick={() => set("tasks", state.tasks.map((item) => item.id === task.id ? { ...item, done: !item.done } : item))}>
          ...
          <article className="panel">
            ...
            <label className="inline-edit">Meta de reuniões <NumberField value={state.meetingsGoal} onChange={(value) => set("meetingsGoal", value)} /></label>
          </article>
        </section>
        ...
        <section className="panel">
          ...
          <div className="scoreboard">
            <div className="score-head"><span>Mês</span><span>Receita</span><span>Reuniões</span><span>Clientes</span><span>Perdidos</span><span>Ticket</span></div>
