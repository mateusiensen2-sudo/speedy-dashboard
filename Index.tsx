...
const monthOnlyKey = (value: string) => monthKey(value.split("/")[0].split(" ")[0]);
...
const DEFAULTS: AppState = {
  ...
  br: { investment: 0, leads: 0, qualified: 0, meetingsBooked: 0, meetingsDone: 0, proposals: 0, closed: 0, ticket: 0, averageInitial: 0, averageMrr: 0, averageContractMonths: 0, immediateRevenue: 0, newMrr: 0, exchangeRate: 0, investmentBrl: 0, immediateRevenueBrl: 0, newMrrBrl: 0 },
  us: { investment: 0, leads: 0, qualified: 0, meetingsBooked: 0, meetingsDone: 0, proposals: 0, closed: 0, ticket: 0, averageInitial: 0, averageMrr: 0, averageContractMonths: 0, immediateRevenue: 0, newMrr: 0, exchangeRate: 0, investmentBrl: 0, immediateRevenueBrl: 0, newMrrBrl: 0 },
...
const COMMERCIAL_SHEET_CONFIG = {
  ...
  brTotalRange: "D77:W77",
  usTotalRange: "D116:W116",
};
...
function readGoogleRows(spreadsheetId: string, sheet: string, range: string): Promise<unknown[][]> {
  return new Promise((resolve, reject) => {
    ...
    (window as any)[callbackName] = (response: GoogleResponse) => {
      ...
      resolve(rows.map((row) => (row.c || []).map((cell) => cell?.v ?? cell?.f ?? "")));
    };
...
async function readGoogleRange(spreadsheetId: string, sheet: string, range: string): Promise<unknown[]> {
  const rows = await readGoogleRows(spreadsheetId, sheet, range);
  return rows[0] || [];
}

function normalizeSheetText(value: unknown) {
  return String(value || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
function isRecurringContract(value: unknown) {
  const contract = normalizeSheetText(value);
...
function rowsToRevenueBase(rows: unknown[][], monthSheet: string): RevenueBase {
  ...
  rows.forEach((row) => {
    ...
    if (name && isRecurringContract(contract)) {
      clients.add(normalizeSheetText(name));
...
function sheetTotalToMarket(row: unknown[]): Market {
  ...
  return {
    ...
    exchangeRate: parseSheetNumber(row[16]),
    investmentBrl: parseSheetNumber(row[17]),
    immediateRevenueBrl: parseSheetNumber(row[18]),
    newMrrBrl: parseSheetNumber(row[19]),
  };
}
...
function normalizeState(parsed: Partial<AppState>): AppState {
  ...
  return {
    ...
    revenueBase: { ...DEFAULTS.revenueBase, ...parsed.revenueBase },
...
export default function Index() {
  ...
  const saveLabel = saveStatus === "saving" ? "Salvando..." : saveStatus === "error" ? "Erro ao salvar" : "";
  ...
  const syncGoogleSheets = async (showToast = false) => {
    const selectedMonth = state.selectedMonth || getCurrentMonthSheetName();
    setSheetStatus("loading");

    try {
      const [brRow, usRow, revenueRows] = await Promise.all([
        readGoogleRange(COMMERCIAL_SHEET_CONFIG.id, selectedMonth, COMMERCIAL_SHEET_CONFIG.brTotalRange),
        readGoogleRange(COMMERCIAL_SHEET_CONFIG.id, selectedMonth, COMMERCIAL_SHEET_CONFIG.usTotalRange),
        readGoogleRows(REVENUE_SHEET_CONFIG.id, selectedMonth, REVENUE_SHEET_CONFIG.range),
      ]);

      const nextBr = sheetTotalToMarket(brRow);
      const nextUs = sheetTotalToMarket(usRow);
      const revenueBase = rowsToRevenueBase(revenueRows, selectedMonth);
      ...
      setState((current) => ({
        ...
        months: current.months.map((row) =>
          monthOnlyKey(row.month) === monthOnlyKey(selectedMonthName)
            ? {
                ...row,
                revenue: revenueBase.totalRevenue,
                investment: nextBr.investment + nextUs.investment,
                leads: nextBr.leads + nextUs.leads,
                qualified: nextBr.qualified + nextUs.qualified,
                meetings: nextBr.meetingsDone + nextUs.meetingsDone,
                closed: nextBr.closed + nextUs.closed,
  ...
  const combinedFunnel: Funnel = {
    investment: state.br.investment + state.us.investmentBrl,
    ...
    ticket: (state.br.closed + state.us.closed) > 0
      ? ((state.br.ticket * state.br.closed) + (state.us.ticket * state.us.exchangeRate * state.us.closed)) / (state.br.closed + state.us.closed)
      : 0,
  };
  ...
  const exportSummary = async () => {
    const lines = [
      ...
      "",
      "BASE",
      "Clientes ativos: " + NUM(state.revenueBase.activeClients),
      "Receita recorrente: " + BRL(state.revenueBase.recurringRevenue),
      "Ticket médio: " + BRL(state.revenueBase.averageTicket),
      "",
      "ALAVANCA PRINCIPAL",
      ...
      <option key={month} value={month}>
        {month}
            ...
            <button className="ghost-btn" onClick={() => syncGoogleSheets(true)} disabled={sheetStatus === "loading"}>
              <RefreshCw size={12} />
              {sheetStatus === "loading" ? "Atualizando" : "Atualizar"}
            </button>
        ...
        <div className={"sheet-status tone-" + sheetStatus}>
          {sheetStatus === "success"
            ...
            : sheetStatus === "error"
              ...
              : "Dados comerciais vinculados à planilha"}{saveLabel ? " · " + saveLabel : ""}
        </div>
        ...
        <section className="revenue-panel surface elevated">
          ...
          <div className="revenue-context">
            <MiniStat label="Clientes ativos" value={<span className={moneyClass}>{NUM(state.revenueBase.activeClients)}</span>} />
            <MiniStat label="Receita recorrente" value={<span className={moneyClass}>{BRL(state.revenueBase.recurringRevenue)}</span>} />
            <MiniStat label="Ticket médio" value={<span className={moneyClass}>{BRL(state.revenueBase.averageTicket)}</span>} />
            <MiniStat label="Avulsos/pontuais" value={<span className={moneyClass}>{BRL(state.revenueBase.oneOffRevenue)}</span>} />
          </div>
        </section>
